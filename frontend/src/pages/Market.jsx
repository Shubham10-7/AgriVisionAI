...
export default function Market() {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-5">
      <div><h1 className="text-2xl font-display font-bold text-gray-800">🛒 Smart Agri Market</h1><p className="text-gray-500 mt-1">Fertilizers, pesticides & supplements — recommended for your crop & disease</p></div>
      <div className="flex gap-3 flex-wrap">
        <div className="relative flex-1 min-w-[200px]">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
          <input className="w-full pl-9 pr-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/30" placeholder="Search products..." value={search} onChange={e => setSearch(e.target.value)} />
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map(p => (
            <div key={p.id} className="bg-white rounded-2xl shadow-card p-4 hover:shadow-hover transition">
              ...
              <p className="text-xs text-gray-500 mb-2">For: {p.for_crop.join(", ")}</p>
